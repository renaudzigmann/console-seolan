...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="/">
    <div class="spArborescenceMain">
      ...
      <div class="spArborescence panel-group" id="accordion" role="tablist" aria-multiselectable="false">
        <xsl:apply-templates select="/*/Item[@type = 'Theme']" mode="MenuNiveau1" />
      </div>
    </div>
  </xsl:template>
  ...
  <xsl:template match="Item" mode="MenuNiveau1">
    ...
    <div class="{$classTheme} panel panel-default">
      ...
      <div id="{$collapseId}" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading{$collapseId}">
  ...
  <xsl:template match="Item[@type = 'Sous-theme']" mode="SousItem">
    <li class="spPublicationSousTheme">
      <h3 class="spPublicationDossierTitle">
        <xsl:value-of select="Titre"/>
      </h3>
      <ul class="spPublicationSousThemeItems">
        <xsl:apply-templates select="./Item" mode="SousItem">
          <xsl:sort select="Titre"/>
        </xsl:apply-templates>
      </ul>
  ...
  <xsl:template match="Item[@type = 'Dossier']" mode="SousItem">
