	...
	<xsl:variable name="letters" select="'ABCDEFGHIJKLMNOPQRSTUVWXYZ'"/>
   	...
   	<xsl:template match="/Ressources">
		<div class="spIndexMain">
			...
			<xsl:call-template name="createFilDArianeRessources"/>
			<div class="spCenter">
				<h1>
					<xsl:text>Services de A à Z du guide des </xsl:text>
       	...
       	<li>
        	<xsl:choose>
	        	<xsl:when test="key('ressource',$letter)">
	        		<a href="#sp-letter-{$letter}" title="Services commençant par la lettre {$letter}">
			        	<xsl:value-of select="$letter"/>
	        		</a>
	        	</xsl:when>
	        	<xsl:otherwise>
