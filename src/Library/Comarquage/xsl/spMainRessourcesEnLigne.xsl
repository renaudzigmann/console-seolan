...
<xsl:stylesheet version="1.0" 
    ...
    <xsl:key name="ressource" match="Ressource" use="translate(substring(Titre,1,1),'ÀÁÂÃÄÅÈÉÊËÌÍÎÏÒÓÔÕÖØÙÚÛÜ','AAAAAAEEEEIIIIOOOOOOUUUU')"/>
	<xsl:variable name="letters" select="'ABCDEFGHIJKLMNOPQRSTUVWXYZ'"/>

   	<xsl:template match="/RessourcesEnLigne">
		<div class="spIndexMain">
	 		<xsl:call-template name="getBarreThemes"/>
			<xsl:call-template name="createFilDArianeRessourcesEnLigne"/>
			<div class="spCenter">
				<h1>
					<xsl:text>Services en ligne de A à Z du guide des </xsl:text>
			...
			<xsl:apply-templates select="Ressource[generate-id(.)=generate-id(key('ressource',translate(substring(Titre,1,1),'ÀÁÂÃÄÅÈÉÊËÌÍÎÏÒÓÔÕÖØÙÚÛÜ','AAAAAAEEEEIIIIOOOOOOUUUU'))[1])]">
    	...
    	<xsl:if test="contains($letters,$letter)">
	    	<div class="spIndex">
	    		...
	    		<ul class="spPublicationNoeud">
			        <xsl:for-each select="key('ressource',$letter)">
    ...
    <xsl:template name="createIndex">
    	...
    	<xsl:variable name="letter" select="substring($letters,$index,1)"/>
       	<li>
        	<xsl:choose>
	        	<xsl:when test="key('ressource',$letter)">
	        		<a href="#sp-letter-{$letter}" title="Services en ligne commençant par la lettre {$letter}">
			        	<xsl:value-of select="$letter"/>
	        		</a>
	        	</xsl:when>
	        	<xsl:otherwise>
