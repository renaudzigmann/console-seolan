 ...
 */
class ElectronicSignatureWebService
{
  /** @var bool Document embarqué en base64 dans le flux XML. */
  private bool $embedDocument = true;

  /** @var string Url du webservice XML-RPC Universign. */
  private string $url;

  /** @var string  Identifiant du webservice Universign. */
  private string $login;

  /** @var string Mot de passe du webservice Universign */
  private string $password;

  /**
   * Constructeur par défaut pour l'accès au webservice d'Universign.
   *
   * @param string $url Url du webservice XML-RPC
   * @param string $login Identifiant Universign
   * @param string $password Mot de passe Universign
   */
   ...
   * Dans le cadre de la console, les documents sont systématiquement encodés dans la requète en base 64.
   *
   * @return void
   */
  ...
  public function generateXMLRequestTransaction(array $data): string {
    ...
    $arrayNode = $domTree->createElement("array");
    $dataNode = $arrayNode->appendChild($domTree->createElement("data"));
    foreach ($data['pdfs'] as $index => $description) {

      if( is_string($description) ){
        // La description n'est composée que du path
        $filePathOrUrl = $description;
        $filename = basename($filePathOrUrl);
      }
      else if( is_array($description) ) {
        // La description est composée du path et d'un nom de fichier
        $filePathOrUrl = $description['path'];
   ...
   * @return void
   */
  private function signer(DOMDocument $dom, DOMNode $node, $firstname = '', $lastname = '', $emailAddress = '', $successURL, $cancelURL, $failURL, $certificateType = 'simple'): void {
  ...
  private function document(DOMDocument $dom, DOMNode $node, string $url, string $data64, string $name, array $signers): void {
    ...
    // TODO: Voir pour gérer le décalage des champs signature
