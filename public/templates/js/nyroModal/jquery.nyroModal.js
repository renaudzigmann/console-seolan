...
jQuery(function($, undefined) {
	$.nmFilters({
		gallery: {
			is: function(nm) {
			  var datasrc = nm.opener.data('gal');
			  var workwithDataSrc = false;
			  if(datasrc==undefined){
				  var ret = nm.opener.is('[rel]:not([rel=external], [rel=nofollow])');
				}else{
				  workwithDataSrc = true;
				  var rel = nm.opener.data('gal');
				  ret = true;
				}
				if (ret) {				  
				  if(rel==undefined){
					...
					var indexSpace = rel.indexOf(' ');
					var gal = indexSpace > 0 ? rel.substr(0, indexSpace) : rel;
					if(workwithDataSrc){
  					var links = $('[href][data-gal="'+gal+'"], [href][data-gal^="'+gal+' "]');
					}else{
					  var links = $('[href][rel="'+gal+'"], [href][rel^="'+gal+' "]');
					}
					if (links.length < 2)
						ret = false;
			...
			initElts: function(nm) {
			  ...
			  if(rel==undefined){
				...
				if(workwithDataSrc){
				  nm.store.galleryLinks = $('[href][data-gal="'+nm.store.gallery+'"], [href][data-gal^="'+nm.store.gallery+' "]');
				}else{
