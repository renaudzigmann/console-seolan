<?php
// remplacer $drawKey = drawEventTimes... par $drawKey=array('draw_start'=>$start_time,'draw_end'=>$end_time);
// commenter le if($time_key > -1) checkOverlap($this_date_tmp, $time_key, $uid);
// ajouter false && au deux if isset($start_time) && isset($end_time), isset($start_unixtime,$end_unixtime) && date('Ymd',$start_unixtime..
// ajouter exdate=$except_dates au tableau eet rrule=$rrule et alarm
// count 100000 par 52
// remplacer les unset par des =null (unset foirreux sur global)
// desactiver le support de la classe
// FREQ ajouter FREQ_TYPE aux données recur
// Supprimer serialize sur org et att
// Ajout $modifier, tzrevent et tzrgotonext et appel checkEvent si pas tzrchecked
// case UNTIL : $recur_array[($start_date)][($hour.$minute)][$uid]['recur'][$key] = $until;
...
if (false && isset($start_time) && isset($end_time)) {
...
if (false && isset($start_unixtime,$end_unixtime) && date('Ymd',$start_unixtime) < date('Ymd',$end_unixtime)) {
  $spans_day = true;
...
$drawKey=array('draw_start'=>$start_time,'draw_end'=>$end_time);
...
foreach ($rrule_array as $key => $val) {
  ...
  case 'FREQ':
    ...
    $recur_array[($start_date)][($hour.$minute)][$uid]['recur']['FREQ_TYPE'] = $freq_type;
    break;
  case 'COUNT':
    $count = $val;
  ...
  case 'UNTIL':
    ...
    if(substr($val,0,4)>2037) $val='2038-01-01 00:00:00';
    ...
    $recur_array[($start_date)][($hour.$minute)][$uid]['recur'][$key] = $regs[1].'-'.$regs[2].'-'.$regs[3];
    break;
  case 'INTERVAL':
    if ($val > 0){
      $interval = $val;
...
if($count == 52 && $interval == 1 && $mArray_begin > $next_range_unixtime) $next_range_unixtime = $mArray_begin;
...
if($end_range_unixtime>2145913200) $end_range_unixtime=2145913200; // La dete de fin ne peux exeder le 1/1/2038
...
foreach($recur_data as $recur_data_unixtime) {
  ...
  while (date("Ymd", $next_range_unixtime) <= $end_date_tmp) {
    ...
    if(!isset($master_array[$this_date_tmp][$time_key][$uid]['sequence']) || 
      ...
      if(empty($module->tmp->tzrchecked)) checkEvent($module,$uid,$modified,true);
      $master_array[$this_date_tmp][$time_key][$uid] = array (
							      ...
							      'alarm'=>$trigger,
							      'modified'=>$modified,
							      'tzrevent'=>$module->tmp->tzrevent,
							      'tzrgotonext'=>$module->tmp->tzrgotonext
							      );
      $tzrfirst=false;
    }
...
$start_time= $start_time_tmp= $end_time= $end_time_tmp= $start_unixtime= $start_unixtime_tmp= $end_unixtime= $end_unixtime_tmp= $summary= $length= $description= $status= $class= $location= $organizer= $attendee=null;
