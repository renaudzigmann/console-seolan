...
class ICal
{
    ...
    public function __construct($filename)
    {
        ...
        if (stristr($lines[0], 'BEGIN:VCALENDAR') === false) {
            ...
            for($i = 0; $i < count($lines);$i++) {
                ...
                $nextline = $lines[$i+1];
                mb_ereg_replace("[\r\n]", "", $nextline);
                if(isset($nextline{0}) && ($nextline{0} == " " || $nextline{0} == "\t")) {
                   $lines[$i] = $lines[$i] . substr($nextline, 1);
                   unset($lines[$i+1]);
            }
                                                        ...
                                                        $value)
    {
        ...
        case "VEVENT":
            ...
            elseif (0 === strpos($keyword, 'ATTENDEE')) {
                $this->cal[$component][$this->event_count - 1]['ATTENDEE'] .= mb_ereg_replace('mailto:', '', $value).';';
            }
            //elseif ($keyword === 'CATEGORIES') {
            //   $this->cal[$component][$this->event_count - 1]['CATEGORIES'][] = $value;
            //}
            else {
            $this->cal[$component][$this->event_count - 1][$keyword] = $value;
            }
