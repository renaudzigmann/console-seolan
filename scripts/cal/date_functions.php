...
function localizeDate($format, $timestamp) {
	...
	$date = str_ireplace('%Y', $year, $format);
	$date = str_ireplace('%e', $day, $date);
	$date = str_ireplace('%B', $monthsofyear_lang[$month], $date);
	$date = str_ireplace('%b', $monthsofyearshort_lang[$month], $date);
	$date = str_ireplace('%A', $daysofweek_lang[$dayofweek], $date);
	$date = str_ireplace('%a', $daysofweekshort_lang[$dayofweek], $date);
...
function extractDateTime($data, $property, $field) {
	...
	$field = str_ireplace(';VALUE=DATE-TIME', '', $field); 
	...
	}else
	{
		...
		$time = $regs[4] . $regs[5];
		$unixtime = mktime($regs[4], $regs[5], $regs[6], $regs[2], $regs[3], $regs[1]);
		...
		date_default_timezone_set($tz_dt);
		...
		// Set the values.
		if($offset_tmp == ($offset=str_ireplace('+','-',$offset_tmp)))
		{
			$offset=str_ireplace('-','+',$offset_tmp);
		}
	
		$unixtime = strtotime($offset,$unixtime);
		$date = date('Ymd', $unixtime);
